...
export class Pattern {
    ...
    ensureCanMatch() {
        this.getElements().forEach(element => assert(!element?.Node?.MatchedNode, 'Matched nodes cannot be in input patterns: ' + JSON.stringify(element)));
    }
    toEngineJSON() {
        ...
        await elementNodes.reduce(async (prev, node) => {
            ...
            if (!isRelation(node)) {
                ...
                if (metaType === 'Node') { // Short-hand for AnyNode with a base pointer
                    ...
                    pattern.addRelation(ptrIndex, ptrNameIndex, Relation.With(Property.Name, Property.Value));
                    pattern.addRelation(ptrIndex, baseIndex, Relation.With(Property.Value, Property.Value));
                }
                else {
